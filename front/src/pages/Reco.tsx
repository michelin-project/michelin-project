    ...
    <div className="pb-10">
      <div className="px-6 pt-6">
        ...
        <h1 className="mt-1 text-[26px] font-black tracking-tight leading-tight">
          Le pneu fait pour{" "}
          <span className="text-primary">{archetype.name}</span>
        </h1>
      </div>
      ...
      <div className="mt-6 px-6">
        <div className="flex items-baseline justify-between">
          <h3 className="text-[13px] font-bold uppercase tracking-wider">
            Autres pneus compatibles
          </h3>
          <span className="text-[11px] text-muted-foreground">
            {others.length} modèles
          </span>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="p-3">
                ...
                <div className="mt-1 font-bold text-sm tracking-tight">
                  {t.name}
                </div>
      ...
      <div className="px-6 mt-6 space-y-3">
        <button
          onClick={onJoin}
          className="w-full h-14 rounded-2xl bg-primary text-primary-foreground font-semibold text-[15px] shadow-[var(--shadow-elevated)] flex items-center justify-center gap-2"
        >
          Rejoindre le challenge — débloquer −15%
        </button>
        <button
          onClick={onSkipBuy}
          className="w-full h-12 rounded-2xl bg-background border border-border font-medium text-sm"
        >
