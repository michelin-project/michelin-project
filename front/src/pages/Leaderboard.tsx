  ...
  return (
    <div className="px-6 pt-4 pb-10">
      ...
      <p className="text-sm text-muted-foreground mt-1">
        Vos amis cyclistes — Challenge 200 km
      </p>

      <div className="mt-5 flex rounded-full bg-surface p-1 text-xs font-semibold">
        <button className="flex-1 h-9 rounded-full text-muted-foreground">
          Amis
        </button>
        <button className="flex-1 h-9 rounded-full bg-background shadow-sm">
          Global
        </button>
        <button className="flex-1 h-9 rounded-full text-muted-foreground">
          Club
      ...
      {rows.length > 0 && (
        <div className="mt-5 grid grid-cols-3 gap-3 items-end">
          {second ? (
            <PodiumItem rank={2} name={second.name} km={second.km} h="h-20" />
          ) : (
            <div />
          )}
          {first ? (
          ...
          {third ? (
            <PodiumItem rank={3} name={third.name} km={third.km} h="h-16" />
          ) : (
            <div />
          )}
        </div>
      ...
      <div className="mt-6 rounded-2xl border border-border divide-y divide-border overflow-hidden">
        {rows.map((r) => {
          ...
          return (
            ...
            >
              <div
                className={`w-7 h-7 rounded-full grid place-items-center text-[11px] font-bold ${r.rank <= 3 ? "bg-michelin-yellow text-ink" : "bg-surface text-ink-soft"}`}
              >
                {r.rank}
              </div>
              ...
              <button className="text-[11px] font-semibold text-muted-foreground">
                Partager
              </button>
            </div>
          );
