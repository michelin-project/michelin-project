  ...
  return (
    <div className="flex items-center gap-3 px-5 py-4 border-b border-border flex-shrink-0">
      <div className="flex items-center gap-2 flex-1">
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${step >= 1 ? "bg-blue-600 text-white" : "bg-muted text-muted-foreground"}`}
        >
          1
        </div>
        <div
          className={`flex-1 h-1 rounded-full transition-colors ${step >= 2 ? "bg-blue-600" : "bg-muted"}`}
        />
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${step >= 2 ? "bg-blue-600 text-white" : "bg-muted text-muted-foreground"}`}
        >
          2
        </div>
      </div>
...
export function MichelinMark({ className = "" }: { className?: string }) {
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-black text-[10px] tracking-tight">
        M
      </div>
...
export function BottomNav({
  ...
  const items: { key: ScreenKey; label: string; icon: string }[] = [
    // { key: "home", label: "Accueil", icon: "M3 11.5 12 4l9 7.5V20a1 1 0 0 1-1 1h-5v-6h-6v6H4a1 1 0 0 1-1-1z" },
    {
      key: "challenge",
      label: "Challenge",
      icon: "M12 2 14.39 8.26 21 9l-5 4.87L17.18 21 12 17.77 6.82 21 8 13.87 3 9l6.61-.74z",
    },
    { key: "progress", label: "Progression", icon: "M3 17 9 11l4 4 8-8" },
    {
      key: "shop",
      label: "Boutique",
      icon: "M6 6h15l-1.5 9h-12zM6 6 5 3H2m4 17a1 1 0 1 0 0-2 1 1 0 0 0 0 2m11 0a1 1 0 1 0 0-2 1 1 0 0 0 0 2",
    },
  ];
  return (
...
export function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-full bg-surface px-3 py-1 text-[11px] font-medium text-ink-soft capitalize">
      {children}
    </span>
  );
}
...
export function RadarChart({
  ...
  scores: { perf: number; end: number; adv: number; ctrl: number };
}) {
  const size = 180;
  const cx = size / 2,
    cy = size / 2,
    ...
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-44">
      {[0.33, 0.66, 1].map((t) => (
        <polygon
          ...
          points={angles
            .map(
              (a) => `${cx + r * t * Math.cos(a)},${cy + r * t * Math.sin(a)}`,
            )
            .join(" ")}
          fill="none"
          stroke="white"
      ...
      <path
        d={path}
        fill="oklch(0.89 0.18 95 / 0.4)"
        stroke="oklch(0.89 0.18 95)"
        strokeWidth={2}
      />
      {points.map((p, i) => (
...
export function PodiumItem({
  ...
  return (
    <div className="text-center">
      <div className="mx-auto mb-2 w-12 h-12 rounded-full bg-surface grid place-items-center font-bold">
        {name[0]}
      </div>
      <div className="text-[11px] font-semibold">{name}</div>
      <div className="text-[10px] text-muted-foreground">{km} km</div>
      <div
        className={`${h} mt-2 rounded-t-xl ${gold ? "bg-michelin-yellow" : "bg-surface"} flex items-start justify-center pt-2 font-black`}
      >
        {rank}
      </div>
    </div>
